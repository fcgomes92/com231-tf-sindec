...
from sindec.forms.user import LoginForm, PasswordResetRequestForm, RegisterRequestForm, SetPasswordForm
...
class RegisterRequestView(FormView, UsersViews):
    ...
    '''
    template_name = 'sindec_base/register.html'
...
##
class PasswordResetRequestView(FormView, UsersViews):
    '''
    View to reset a password
    Config vars:
        > template_name     :: Template to be loades
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            data = False
        if self.check_user_input(data) is True:
            ...
            if associated_users.exists():
                for user in associated_users:
                    ...
                    subject = self.subject
                    email = loader.render_to_string(self.email_template, c)
                    mail = EmailMessage(subject, email, self.from_email, [user.email])
                    mail.content_subtype = "html"
                    try:
                        mail.send()
                        result = self.form_valid(form)
                        messages.success(request, strings.RESET_PASSWORD_CONFIRM)
                    except Exception as e:
                        print(e)
                        messages.success(request, strings.RESET_PASSWORD_ERR_SEND_EMAIL)
                        result = self.form_invalid(form)
                result = self.form_valid(form)
