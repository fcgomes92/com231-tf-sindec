from sindec.models import Assunto, Reclamacao, Problema, Empresa, CNAE
...
def relatorio_do_cadastro_nacional_de_reclamacoes_fundamentadas(context=None, ano=2009):
    ...
    columns = ["CNPJ", "Razão Social", "Reclamações Atendidas", "Reclamações Não Atendidas", "Total de Reclamações"]
    qtd_reclamacoes = list()
        ...
        qtd_reclamacoes.append(
            {
                ...
                "data": [
                    ...
                    "{} ({})%".format(atendida, ((atendida / total) * 100)),
                    "{} ({})%".format(naoatendida, ((naoatendida / total) * 100)),
                    "{} ({})%".format(total, (((total / len(reclamacoes)) * 100))),
                ],
            }
        )
